...
//	TODO: add in optional callback to all functions
	this.makeAuthenticatedRequest = function(endpoint, params, nonce, cb){
		...
		var payload = {
			'request': '/v1' + endpoint,
			'nonce':  nonce?nonce.toString():new Date().getTime().toString()
		};
...
//	Authenticated requests (must have API_KEY and API_SECRET)
	...
	this.getWalletBalances = function(nonce){
		var endpoint = '/balances';
		var result = _this.makeAuthenticatedRequest(endpoint, {}, nonce);
		return result.data;
	};
    ...
    this.getSymbols=function(){ //add missing functionality to the Bitfinex pacakge
        var response = HTTP.get(this.url + '/symbols');
		var a = response.data.map(function(x){return x.toUpperCase();});
		return a;
    }
    this.getCurrencies=function(){ //add missing functionality to the Bitfinex pacakge
        var response = HTTP.get(this.url + '/symbols');
        var currs={};
        var finalCurrency=[];
        var n=response.data.length;
        var lambda=function(curr){
            if(!currs[curr]){
                currs[curr]=1;
                finalCurrency.push(curr.toUpperCase());
            } 
        }
        for(var i=0; i<n; i++){
            var firstCurr=response.data[i].substring(0, 2);//gets  first three digits
            var secondCurr=response.data[i].substring(3);
            lambda(firstCurr);
            lambda(secondCurr);
        }
